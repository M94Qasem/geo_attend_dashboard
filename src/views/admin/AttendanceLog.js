...
export default function AttendanceLog() {
  ...
  const columns = useMemo(() => [
    { 
      ...
      header: 'Employee', 
      cell: info => (
        <span className="font-bold cursor-pointer hover:text-sky-500 dark:hover:text-sky-400 transition-colors" onClick={() => openDetailsPanel(info.row.original)}>
          {info.getValue()}
        </span>
      ) 
    },
    ...
    { 
      ...
      cell: info => {
        ...
        let colorClasses = "";
        // ✅ تعديل ألوان الحالة للوضع الليلي
        switch (status) {
          case "On Time": colorClasses = "bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-400"; break;
          case "Late": colorClasses = "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-400"; break;
          case "Override": colorClasses = "bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-400"; break;
          case "Absent": colorClasses = "bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-400"; break;
          case "Failure": colorClasses = "bg-purple-100 text-purple-800 dark:bg-purple-900/50 dark:text-purple-400"; break;
          default: colorClasses = "bg-gray-100 text-gray-800 dark:bg-slate-700 dark:text-slate-300";
        }
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex gap-3 justify-center">
          <button className="text-green-500 hover:text-green-700 dark:hover:text-green-400 transition-colors" title="Approve" onClick={() => handleAction('approve', row.original)}><FaCheck /></button>
          <button className="text-red-500 hover:text-red-700 dark:hover:text-red-400 transition-colors" title="Reject" onClick={() => handleAction('reject', row.original)}><FaTimes /></button>
          <button className="text-yellow-500 hover:text-yellow-700 dark:hover:text-yellow-400 transition-colors" title="Add Manual Entry" onClick={() => handleAction('manual', row.original)}><FaPlus /></button>
        </div>
      )
    },
  ], []);
 ...
 return (
    <>
      {/* ✅ تعديل الهيدر للوضع الليلي والطباعة */}
      <div className="relative bg-sky-600 dark:bg-slate-700 md:pt-32 pb-32 pt-12 print:hidden">
        <div className="px-4 md:px-10 mx-auto w-full">
          <div>
            <div className="flex flex-wrap">
              <div className="w-full lg:w-6/12 xl:w-3/12 px-4">
                <CardStats statSubtitle="ON TIME" statTitle="350,897" statArrow="up" statPercent="3.48" statPercentColor="text-emerald-500" statDescripiron="Since last month" icon="far fa-chart-bar" color="green" />
              </div>
              <div className="w-full lg:w-6/12 xl:w-3/12 px-4">
                <CardStats statSubtitle="LATE" statTitle="2,356" statArrow="down" statPercent="3.48" statPercentColor="text-red-500" statDescripiron="Since last week" icon="fas fa-chart-pie" color="orange" />
              </div>
              <div className="w-full lg:w-6/12 xl:w-3/12 px-4">
                <CardStats statSubtitle="ABSENT" statTitle="924" statArrow="down" statPercent="1.10" statPercentColor="text-orange-500" statDescripiron="Since yesterday" icon="fas fa-users" color="pink" />
              </div>
              <div className="w-full lg:w-6/12 xl:w-3/12 px-4">
                <CardStats statSubtitle="OVERRIDE" statTitle="49,65%" statArrow="up" statPercent="12" statPercentColor="text-emerald-500" statDescripiron="Since last month" icon="fas fa-percent" color="sky" />
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ✅ تعديل حاوية المحتوى للطباعة */}
      <div className="px-4 md:px-10 mx-auto w-full -m-24 print:m-0 print:p-0">
        <div className="flex flex-wrap mt-4">
          <div className="w-full mb-12">
            <CardTable 
              color="light" // سيتم تجاهل هذا لأن CardTable الآن يدعم الوضع الليلي مباشرة
              data={filteredData} 
              columns={columns} 
      ...
      {/* ✅ تعديل المودال للوضع الليلي (نفترض أن المكون الداخلي تم تعديله) */}
      ...
      {/* ✅ تعديل اللوحة الجانبية للوضع الليلي */}
      <Transition.Root show={panel.isOpen} as={Fragment}>
        <Dialog as="div" className="relative z-40" onClose={() => setPanel({ isOpen: false, record: null })}>
          <Transition.Child as={Fragment} enter="ease-in-out duration-500" enterFrom="opacity-0" enterTo="opacity-100" leave="ease-in-out duration-500" leaveFrom="opacity-100" leaveTo="opacity-0">
            <div className="fixed inset-0 bg-gray-500 bg-opacity-75 dark:bg-black/80 transition-opacity" />
          </Transition.Child>
                ...
                <Transition.Child as={Fragment} enter="transform transition ease-in-out duration-500" enterFrom="translate-x-full" enterTo="translate-x-0" leave="transform transition ease-in-out duration-500" leaveFrom="translate-x-0" leaveTo="translate-x-full">
                  <Dialog.Panel className="pointer-events-auto w-screen max-w-md">
                    <div className="flex h-full flex-col overflow-y-scroll bg-white dark:bg-slate-800 shadow-xl">
                      <div className="bg-sky-700 dark:bg-sky-900 py-6 px-4 sm:px-6">
                        ...
                        <p className="mt-1 text-sm text-sky-300 dark:text-sky-400">Details for <strong className="text-white">{panel.record?.name}</strong></p>
                      </div>
                      <div className="relative flex-1">
                        <Tab.Group>
                          <Tab.List className="flex space-x-1 rounded-t-xl bg-sky-900/20 dark:bg-slate-900/50 p-1">
                            <Tab className={({ selected }) => `w-full rounded-lg py-2.5 text-sm font-medium leading-5 transition-colors ${selected ? 'bg-white dark:bg-slate-700 shadow text-sky-700 dark:text-white' : 'text-blue-100 hover:bg-white/[0.12] dark:text-sky-300 dark:hover:bg-slate-900/75'}`}>Details</Tab>
                            <Tab className={({ selected }) => `w-full rounded-lg py-2.5 text-sm font-medium leading-5 transition-colors ${selected ? 'bg-white dark:bg-slate-700 shadow text-sky-700 dark:text-white' : 'text-blue-100 hover:bg-white/[0.12] dark:text-sky-300 dark:hover:bg-slate-900/75'}`}>Activity Log</Tab>
                          </Tab.List>
                          <Tab.Panels className="mt-2 p-4">
                            <Tab.Panel>
                              ...
                              <div className="mt-6 h-64 bg-gray-200 dark:bg-slate-700 rounded-lg flex items-center justify-center">
                                <p className="text-gray-500 dark:text-slate-400">[Map Component Placeholder]</p>
                              </div>
                            </Tab.Panel>
                            <Tab.Panel>
